...
> For design details, QLoRA configuration, LoRA adapter parameters, training hyperparameters, and data collation internals, see the [Training Pipeline architecture](../architecture/ground_segment/training.md) page.
...
- For detailed GPU, RAM, and disk requirements, see [Compute Budgets](../architecture/ground_segment/budgets.md).
...
Move the data and the code to the training server using `upload_to_server.sh` (see [scripts.md](../architecture/ground_segment/scripts.md)). On the server, create a new environment and run fine tuning:
...
Finally, use `download_weights.sh` to transfer the weights from the training server to your local machine in `ground_segment/training` (see [scripts.md](../architecture/ground_segment/scripts.md)).
