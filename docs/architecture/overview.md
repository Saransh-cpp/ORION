...
| **CameraManager**       | Earth observation camera payload     | Active  | 30       | Acquires imagery from SimSat's Mapbox API. Fuses GPS telemetry and dispatches to the VLM queue. Auto-capture at 85s intervals in MEASURE mode. |
...
| -------------- | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SimSatClient` | NavTelemetry, CameraManager | Stateless libcurl wrapper for the SimSat REST API. `fetchPosition()` returns lat/lon/alt; `fetchMapboxImage()` fetches a 1280x1280@2x PNG Mapbox tile, decodes it via vendored `stb_image`, bilinear-resizes to 512x512 via `stb_image_resize2`, and writes raw RGB into a caller-provided buffer. The 512x512 target matches typical smallsat camera resolution and keeps memory bounded on the Pi 5; in a real mission the camera would output directly at native resolution with no resize. |
